...
export const getDocument = async (req, res, next) => {
    ...
    try {
        ...
        const findProductInOrder = order.products.find(item => item === productType.id);
...
export const getDocuments = async (req, res, next) => {
    ...
    try {
        ...
        const requests = products.map(item => getProductTypesByIdService(item));
        const results = await Promise.all(requests);

        const requestsProduct = results.map(item => getProductByIdService(item.productId));
        const resultsProduct = await Promise.all(requestsProduct);
        
        const filesProducts = resultsProduct.filter(item => item.file_id !== null);

        if (filesProducts.length === 0) {
            return handleResponse(res, 404, "No documents found for products in this order");
        }
        ...
        return handleResponse(res, 200, 'Document succesfully fetched',{ documents });
    } catch (err) {
        next(err);
    }
}
