...
export const getAllProductService = async (page) => {
    const result = await pool.query('SELECT * FROM products WHERE page=$1', [
        page,
    ]);
...
export const updateProductByIdService = async (id, data) => {
    const req = await pool.query(
        'UPDATE products SET name=$1, description=$2, short_description=$3, category=$4, page=$5, tag=$6, file_id=$7, image=$8, size_image=$9 WHERE id=$10 RETURNING *',
        [
            data.name,
            data.description,
...
export const deleteProductWithProductTypesService = async (id) => {
    ...
    const promises = get.rows.map((element) => {
        return pool.query('DELETE FROM "productType" WHERE id=$1', [
            element.id,
        ]);
    });
...
export const getDocumentService = async (id) => {
    const req = await pool.query('SELECT * FROM files WHERE id=$1', [id]);
...
export const getProductTypesByIdService = async (id) => {
    const req = await pool.query('SELECT * FROM "productType" WHERE id=$1', [
        id,
    ]);
    const result = req.rows[0];
...
export const getProductByIdProductTypesService = async (id) => {
    ...
    const prodRes = await pool.query('SELECT * FROM products WHERE id=$1', [
        req.rows[0].productId,
    ]);
...
export const createProductWithProductsTypesService = async (
    product,
    productTypes,
) => {
    ...
    const promises = productTypes.map((element) => {
        return pool.query(
            ...
            [
                ...
                element.color,
                element.images,
                element.size_placeholder,
                element.stock_quantity,
                resultProduct.rows[0].id,
            ],
        );
    });
    ...
    return {
        ...
        productTypes: resultsProductTypes,
    };
};
