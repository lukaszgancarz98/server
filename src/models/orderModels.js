...
export const getAllOrdersService = async (page) => {
    const result = await pool.query('SELECT * FROM orders WHERE page=$1', [
        page,
    ]);
    return result.rows;
};
...
export const getOrderByEmailService = async (email) => {
    const req = await pool.query('SELECT * FROM orders WHERE email = $1', [
        email,
    ]);

    const result = req.rows.filter(
        (item) => !item.finalize_date && !item.payment_date,
    );
...
export const updateOrderDetailsService = async (id, orderDetails) => {
    ...
    const req = await pool.query(
        'UPDATE orders SET "orderDetails"=$1 WHERE id=$2 RETURNING *',
        [data, id],
    );
...
export const updateOrderPaymentToken = async (data, id) => {
    ...
    const req = await pool.query(
        'UPDATE orders SET token=$1, token_expire_date=$2 WHERE id=$3 RETURNING *',
        [access_token, now, id],
    );
...
export const getOrderPaymentToken = async (id) => {
    ...
    return {
        token: result?.token,
        expire: result?.token_expire_date,
        payment: result?.payment_date,
    };
};
...
export const getOrderPaymentTokenAndId = async (id) => {
    ...
    return {
        token: result.token,
        expire: result.token_expire_date,
        paymentorder_id: result.paymentorder_id,
    };
};
    ...
    if (req.rows.length > 0) {
        ...
        if (dbAmount > 0) {
            ...
            const response = await pool.query(
                'UPDATE "productType" SET sale_amount=$1 WHERE id=$2 RETURNING *',
                [calculatedAmount, id],
            );
